AFRAME.registerComponent("create-markers", {
  ...
  init: async function() {
    ...
    dishes.map(dish => {
      ...
      titlePlane.setAttribute("position", { x: 7, y: 0.89, z: 0.02 });
      titlePlane.setAttribute("rotation", { x: 0, y: 0, z: 0 });
      titlePlane.setAttribute("width", 1.69);
      titlePlane.setAttribute("height", 0.3);
      titlePlane.setAttribute("material", { color: "#fff" });
      mainPlane.appendChild(titlePlane);

      // Dish title
      var dishTitle = document.createElement("a-entity");
      dishTitle.setAttribute("id", `dish-title-${dish.id}`);
      dishTitle.setAttribute("position", { x: 7, y: 0, z: 0.1 });
      ...
      ingredients.setAttribute("position", { x: 7.5, y: 0, z: 0.1 });
